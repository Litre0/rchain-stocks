...
> **Refreshing is slow, and the liveness window is why.** All measured on a real run:
> `pools.py` 27 min and `symbols.py` 17 min on a cold start only — both checkpoint, so later
> runs skip them. But `live.py` keeps **no cursor** and re-sweeps its whole window every single
> run: a 6h window is 213,380 blocks and took **46 min**, warm or cold. It is not hung.
>
> Cost is linear in the window, and that is the only real lever:
>
> | `--window` | blocks | live.py |
> |---|---|---|
> | `30m` | ~18k | ~4 min |
> | `1h` | ~36k | ~8 min |
